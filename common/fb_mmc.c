		...
		lbaint_t blkcnt, const void *buffer)
{
	...
	for (i = 0; i < blkcnt; i += FASTBOOT_MAX_BLK_WRITE) {
		...
		if (buffer != NULL) {
			...
			printf("write mmc %d at 0x%08x\n",block_dev->devnum,(unsigned)blk);
			blks_written = blk_dwrite(block_dev, blk, cur_blkcnt,
					buffer+(i*block_dev->blksz));
		} else {
			timed_send_info(&timer, "erasing");
			printf("erase mmc %d at 0x%08x\n",block_dev->devnum,(unsigned)blk);
		...
		unsigned int download_bytes, char *response)
{
	...
	printf("........ wrote " LBAFU " bytes to '%s'(0x%08x)\n", blkcnt * info->blksz,
	       part_name,(unsigned)(info->start));
...
#if 0//  CONFIG_IS_ENABLED(EFI_PARTITION)
...
#if 0 // CONFIG_IS_ENABLED(DOS_PARTITION)
...
download:
	if(fastboot_oem_mode == OEM_MODE_NORMAL){
