...
#define BOOTENV \
	...
	"autostart=y        \0" \
	"init=/sbin/init    \0" \
	"rootfs=/dev/mmcblk0p4 \0" \
	"root=/dev/mmcblk2p4 rootwait rw rootfstype=ext4 \0" \
	"console=earlyprintk console=ttyFIQ0 consoleblank=0 loglevel=7 \0" \
	"booting=unzip ${kernel_addr_c} ${kernel_addr_r}; booti ${kernel_addr_r} ${ramdisk_addr_r} ${fdt_addr_r}; \0" \
	"uncompression=unzip ${kernel_addr_c}  ${kernel_addr_r}\0" \
	"fastboot_run=fastboot usb 2; ${booting};  \0" \
	"automatic_run=pinfastboot; fatload mmc 0:3 ${kernel_addr_c} /uImage.img; fatload mmc 0:3 ${fdt_addr_r} /uImage.dtb;  ${booting}; \0" \
	"partitions=rk29xxnand:0x00004000@0x00002000(uboot),0x00004000@0x00006000(environment),0x00020000@0x0000a000(kernel),-@0x0002a000(rootfs:grow) \0" \
	"bootargsmedia=storagemedia=emmc androidboot.storagemedia=emmc androidboot.mode=normal\0" \
	"setargs=setenv bootargs ${bootargsmedia} rootfs=${rootfs} console=${console}  root=${root}\0" \
	"bootcmd2=run setargs automatic_run \0" \
	"bootcmd=version \0"
	

#ifndef CONFIG_BOOTCOMMAND
#define CONFIG_BOOTCOMMAND "run version"
#endif
