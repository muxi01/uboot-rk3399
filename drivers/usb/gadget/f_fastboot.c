...
static void cb_oem(struct usb_ep *ep, struct usb_request *req)
{
    ...
    if (strncmp("unlock", cmd + 4, 8) == 0) {
    ...
    else if (strncmp("initrd", cmd + 4, 6) == 0){
        fastboot_oem_mode =OEM_MODE_INITRD;
        fastboot_tx_write_str("OKAY");
    }

    else if (strncmp("rkusb", cmd + 4, 5) == 0){
        fastboot_oem_mode =OEM_MODE_RBROM;
        fastboot_tx_write_str("OKAY");
    }
        ...
        if(logtrace_print(fastboot_tx_write_str) < 0) {
            fastboot_tx_write_str("FAIL log buffer print fail");
        } else {
            fastboot_tx_write_str("OKAY");
        }
#else 
        fastboot_tx_write_str("FAIL CONFIG_LOG_TRACE is not enabled");
#endif 
    }

    else if (strncmp("help", cmd + 4, 4) == 0){
        // "kernel.dtb.exit.initrd.rkusb.normal"
        fastboot_tx_write_str("INFO kernel for download kernel img");
        fastboot_tx_write_str("INFO dtb    for download dtb img");
        fastboot_tx_write_str("INFO initrd for downlaod ramdisk img");
        fastboot_tx_write_str("INFO normal for reset fastboot mode");
